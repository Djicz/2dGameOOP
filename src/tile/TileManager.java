...
public class TileManager {
    ...
    public void loadMap(String s) {
        try {
            ...
            while (col < gp.getMaxWorldCol() && row < gp.getMaxWorldRow()) {
                ...
                String[] numbers = line.split(" "); // Tach tung ki tu thong qua dau " "
    ...
    public void draw(Graphics2D g2, int tileSize) {
        // Khoi tao hang cot va vi tri toa do ban dau cua worldMap
        int worldCol = 0;
        int worldRow = 0;

        while(worldCol < gp.getMaxWorldCol() && worldRow < gp.getMaxWorldRow()) {
            // Lay gia tri tai toa do pixel dang o
            int tileNum = mapFromFile[worldRow][worldCol];
            // ???
            ...
            // Tuong tu screenX
            if(worldX + tileSize > gp.getPlayer().worldX - gp.getPlayer().screenX &&
               worldX - tileSize < gp.getPlayer().worldX + gp.getPlayer().screenX &&
               worldY + tileSize > gp.getPlayer().worldY - gp.getPlayer().screenY &&
               worldY - tileSize < gp.getPlayer().worldY + gp.getPlayer().screenY) {
                g2.drawImage(tile[tileNum].image, screenX, screenY, tileSize, tileSize, null); // Ve component map
            } // Chi khi toa do world o trong khu vuc man hinh co the hien thi thi moi in ra
            worldCol++; // Tang cot
            if(worldCol == gp.getMaxWorldCol()) { // Neu cot = 16 (max) => tang hang len, khoi tao lai cot = 0 (toa do y + do lon pixel (=48))
                worldCol = 0;
                worldRow ++;
            }
        }
