    ...
    public int checkObject(Entity entity, boolean player) {
        ...
        for(int i = 0; i < gp.getObj().length; i++) {
            if(gp.getObj()[i] != null) {
                ...
                gp.getObj()[i].solidArea.x += gp.getObj()[i].worldX;
                gp.getObj()[i].solidArea.y += gp.getObj()[i].worldY;

                switch (entity.direction) {
                    case "up":
                        entity.solidArea.y -= entity.speed;
                        if(entity.solidArea.intersects(gp.getObj()[i].solidArea)) {
                            if(gp.getObj()[i].collision == true) {
                                entity.collisionOn = true;
                            }
                            if(player == true) {
                                index = i;
